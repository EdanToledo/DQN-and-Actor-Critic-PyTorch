...
class SumTree(object):
    ...
    def add(self, priority, data):
        ...
        self.data_pointer += 1  # increment the data pointer since we have added data to th current position
...
class Prioritized_Replay_Memory(object):
    ...
    def sample(self, batch_size):
        batch_index, batch_memory, ImportanceSamplingWeights = np.empty((batch_size), dtype=np.int32), np.empty(
            (batch_size), dtype=object), np.empty((batch_size))
